...
function renderNodeBodies(project: ProjectDocument): Record<string, string> {
  ...
  for (const node of project.graph.nodes) {
    ...
    if (node.type === 'final') {
      // Las dos pantallas bespoke de un Final ("Final TOP"/"con fallos")
      // usan texto FIJO de marca (ver `FINAL_TOP_HEADING`/
      // `FINAL_ALTERNATE_HEADING` en `playerIntroTexts.ts`), nunca
      // `body`/`alternateBody` — no hay nada que pre-renderizar aquí. Esos
      // dos campos siguen existiendo en el dominio (gate de la variante
      // alternativa y contenido para `aiReviewExport.ts`), pero como texto
      // plano, no como HTML enriquecido.
      continue
    }
